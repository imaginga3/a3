...
def removeSelectedObject():
    ...
    image = cv.imread('images/still/5-ball.png')
    ...
    windowSize = 7
    Region = image[y0-windowSize:y1+windowSize,x0-windowSize:x1+windowSize,:]
    cv2.imwrite('Region.png',Region)

    #create FillRegion matrix, which comprises x,y coords to be filled
    newYRange = y1-y0
    newXRange = x1-x0


    #fillregion is a mask of the area of interest, denotes filled vs. unfilled pixels
    FillRegion = np.empty([newYRange+2*windowSize,newXRange+2*windowSize])#empty([newYRange+1,newXRange+1],dtype=[('priority','Float8'),('')])

    #denote Source Space from Fill Space
    for y in range(0,newYRange+(2*windowSize)):
        for x in range(0,newXRange+(2*windowSize)):

            #Source Space (OUTSIDE BOUNDING BOX)
            if (((y > y0 - windowSize) and (x > x0 - windowSize)) or ((y < y1 + windowSize) and (x < x1 + windowSize))):
                FillRegion[y,x] = 1
    ...
    for y in range(0,newYRange+windowSize):
        for x in range(0,newXRange+windowSize):
